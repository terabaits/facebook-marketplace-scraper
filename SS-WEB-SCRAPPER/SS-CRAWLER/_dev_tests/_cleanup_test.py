...
cur=conn.cursor()
# Delete test brands with no listings
cur.execute("""
    DELETE FROM laptop_reference
    WHERE brand = 'TestBrand'
      AND id NOT IN (SELECT DISTINCT laptop_reference_id FROM laptop_listings WHERE laptop_reference_id IS NOT NULL)
""")
conn.commit()
...
for r in cur.fetchall():
    for k, v in zip(['total','material','usb_count','hdmi_count','resolution','refresh_rate_hz','has_hdmi','has_video_pd_usb_c','has_ethernet','has_touchscreen'], r):
