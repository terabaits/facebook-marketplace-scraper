# -*- coding: utf-8 -*-
"""Carefully apply computers.html UI changes - one block at a time with verification."""
from pathlib import Path

FILE = Path(r"G:\Github\SS-WEB-SCRAPPER\SS-WEBSITE\templates\computers.html")
text = FILE.read_text(encoding="utf-8")
print(f"Initial file size: {len(text)} chars")
...
if old_filter_bar in text:
    ...
    print(f"[1] Replaced main filter bar. Size: {len(text)}")
else:
    print("[1] ERROR: filter bar pattern not found")
...
if old_load_head in text:
    ...
    print(f"[2] Updated loadComputers(). Size: {len(text)}")
...
if insertion_point in text and "function cleanComputerTitle" not in text:
    ...
    print(f"[3] Added helper functions. Size: {len(text)}")
...
if old_thead in text:
    ...
    print(f"[4] Removed Type column. Size: {len(text)}")

# ---------- 5. Replace components block ----------
...
if old_components_block in text:
    ...
    print(f"[5] Replaced components block. Size: {len(text)}")
else:
    print("[5] ERROR: components block not found")
...
new_row = '''            html += `
                <tr class="clickable ${pcType}-row" data-pc-type="${pcType}" onclick="showComputerDetail('${safeListingId}')">
                    ...
                    <td class="comp-actions">
                        <button class="comp-action-btn" title="View details" onclick="event.stopPropagation(); showComputerDetail('${safeListingId}')">\U0001F4C4</button>
...
if old_row in text:
    ...
    print(f"[6] Replaced row body. Size: {len(text)}")
else:
    print("[6] ERROR: row body not found")

# ---------- 7. Add new CSS to the existing <style> block (NOT outside endblock) ----------
# The existing <style> block is at line 2195. Add our new CSS BEFORE the </style> tag of that block.
# Find the existing <style> tag
style_open = '<style>'
# Find the first <style> (not the script-relative one)
first_style_idx = text.find('<style>')
if first_style_idx > 0:
    # Find the matching </style>
    next_style_close = text.find('</style>', first_style_idx)
    if next_style_close > 0:
        new_css = '''
...
.comp-cell-name { min-width: 200px; }
...
.pc-type-chip.prebuilt { background: #fee2e2; color: #b91c1c; }
.pc-type-chip.custom { background: #dbeafe; color: #1d4ed8; }
[data-theme="dark"] .pc-type-chip.prebuilt { background: rgba(239,68,68,0.2); color: #fca5a5; }
[data-theme="dark"] .pc-type-chip.custom { background: rgba(59,130,246,0.2); color: #93c5fd; }
...
.comp-meta { font-size: 0.8rem; color: var(--comp-text); }
...
/* Hide legacy badges inside the listing table (we use comp-* now) */
.clickable .badge { display: none; }
'''
        # Insert the new CSS BEFORE the first </style> tag
        text = text[:next_style_close] + new_css + text[next_style_close:]
        print(f"[7] Added new CSS to existing style block. Size: {len(text)}")
    else:
        print("[7] ERROR: could not find first </style>")
else:
    print("[7] ERROR: could not find first <style>")

FILE.write_text(text, encoding="utf-8")
print(f"\nDone. Final file size: {len(text)} chars")
