...
Barības bloks: Cooler Master V1200 1200W 80+Platinum - lietots;
...
has_integrated = any(re.search(pattern, normalized) for pattern in integrated_patterns)
print(f"has_integrated: {has_integrated}")

# VRAM extraction (simplified)
vram = None
import re
vram_match = re.search(r'(?:rx|rtx|gtx)\s*\d{3,4}.*?(\d+)\s*gb', text_lower)
if vram_match:
    vram = int(vram_match.group(1)) * 1024
print(f"vram: {vram}")

if not has_integrated and not no_gpu:
    print("\nAttempting GPU match...")
    gpu_match = matcher.match(text, "", vram_mb=vram)
