...
-- Populate content_hash for existing rows (safely)
DO $$
BEGIN
    UPDATE listings 
...
EXCEPTION WHEN OTHERS THEN
    RAISE NOTICE 'Error populating content_hash: %', SQLERRM;
END $$;
...
DO $$
BEGIN
    CREATE INDEX IF NOT EXISTS idx_flagged_listings_category 
    ON flagged_listings(category) WHERE is_active = TRUE;
EXCEPTION WHEN OTHERS THEN
...
DO $$
BEGIN
    CREATE INDEX IF NOT EXISTS idx_flagged_listings_active 
    ON flagged_listings(listing_id) WHERE is_active = TRUE;
EXCEPTION WHEN OTHERS THEN
    RAISE NOTICE 'Could not create active index: %', SQLERRM;
END $$;
