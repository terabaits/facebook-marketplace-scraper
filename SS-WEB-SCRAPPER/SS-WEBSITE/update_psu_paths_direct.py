...
def get_local_hash(path, size=(16,16)):
    try:
...
def get_remote_hash(url, size=(16,16)):
    try:
...
def sim(a,b):
    ...
    return sum(1 for x,y in zip(a,b) if abs(x-y)<25) / len(a)

local_hashes = {f: get_local_hash(p) for f,p in files.items()}
